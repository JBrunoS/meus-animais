...
export default function Food() {
  ...
  // 🎯 gera opções dinâmicas (não depende do JSON fixo)
  function generateOptions(correctAnimal) {
    const wrongFoods = animals
      .filter((a) => a.id !== correctAnimal.id)
      .sort(() => Math.random() - 0.5)
      .slice(0, 2)
      .map((a) => a.food);

    return shuffle([correctAnimal.food, ...wrongFoods]);
  }

  // 🔁 atualiza opções ao mudar fase
  useEffect(() => {
    if (current) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>O que esse animal come?</Text>
      ...
      <View style={styles.options}>
        {options.map((opt) => {
          ...
          if (selected === opt) {
            bg = opt === current.food ? "#00C853" : "#D63031";
          }
